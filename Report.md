...
- Merge Sort:
  - MPI:
      - Strong Scaling: With constant problem size and increased number of processors we expected to see a better time. We also expected that there could be diminishing results due to communication overheads. This expectaion was validated with trend from our commputation graph [here](https://github.com/RileySzecsy/CSCE435Project/blob/master/MergeSort/MPI/Plotting/MPI_plots/Strong%20Scaling%20MPI%20%7C%20comp%20%7C%20Size%3A%3A%2016777216.png) for 2^24. We can see that the general trend from the plot is that computaiton time goes down up to 128 threads and then it starts declining at lower rates possibly due to communication overhead. From the whole computation plot [!img](https://github.com/RileySzecsy/CSCE435Project/blob/master/MergeSort/MPI/Plotting/MPI_plots/Strong%20Scaling%20MPI%20%7C%20whole_computation%20%7C%20Size%3A%3A%2016777216.png) we can also see that as the number of processes increases the overall time decreases upto 64 nodes, and then we start getting diminishing returns due to communication overhead.
        
      - Speedup: Since speedup shows us how much faster the algorithm runs on multiple processors compared to one, we were wxpecting to see a more linear or logarithmic graph. This expectation was also validated with the computation graph [here](https://github.com/RileySzecsy/CSCE435Project/blob/master/MergeSort/MPI/Plotting/MPI_plots/Speedup%20MPI%20%7C%20comp%20%7C%20Input%20Type%3A%3A%20Sorted.png) for array size 2^24. However, the the whole_computation graph shown [here] (https://github.com/RileySzecsy/CSCE435Project/blob/master/MergeSort/MPI/Plotting/MPI_plots/Speedup%20MPI%20%7C%20whole_computation%20%7C%20Input%20Type%3A%3A%20Sorted.png) did not scale as expected since data initialization computation time for larger arrays and communication overhead.
